<?php
  ...
  class PorselenSiparis extends DataCommon {
      ...
      public function ekle( $postdata ){
          ...
          $random_fix = $postdata["gid"];

          $siparis_dir = UPLOADS_DIR_PORSELEN . $postdata["gid"];
          mkdir( $siparis_dir , 0777, true);

          // preview upload
          if( !CanvasUpload::action( $postdata["preview"], $siparis_dir . "/SGP.png" ) ){
              $this->return_text = "Bir hata oluştu.[1]";
              return false;
          }

          $Upload = new TempUpload( array("parent_gid" => $postdata["gid"], "item_id" => "PORX"));
          ...
          $db_data_array = array(
              ...
              "isim"                => $postdata["isim"],
              "gid"                 => $random_fix,
              "orjinal_resim_ext"   => $Upload->get_org_ext(),
      ...
      public function duzenle( $postdata ){
          $siparis_dir = UPLOADS_DIR_PORSELEN . $postdata["gid"];
          // preview upload
          if( !CanvasUpload::action( $postdata["preview"], $siparis_dir . "/SGP.png" ) ){
              $this->return_text = "Bir hata oluştu.[1]";
          ...
          $Upload = new TempUpload( array("parent_gid" => $postdata["gid"], "item_id" => "PORX"));
          if( $Upload->is_ok() ){
              $Upload->transfer( $siparis_dir . "/" ); 
              $org_ext = $Upload->get_org_ext();
          } else {
              $org_ext = $this->details["orjinal_resim_ext"];
          }
      ...
      public function sil(){
           ...
           // resimleri sil
           array_map('unlink', glob(UPLOADS_DIR_PORSELEN . $this->details["gid"]."/*.*"));
           rmdir(UPLOADS_DIR_PORSELEN . $this->details["gid"]);
      ...
      // @DEPRECATED
      public function duzenle_old( $postdata, $filesdata ){
      ...
      // @DEPRECATED
      public function ekle_old( $postdata, $filesdata ){
